...
def generateInputXmlFile():
	...
	# the updated input data XML file generated by generateUpdatedInputXmlFile(). The updated input data XML file represents the partial solution
	# and when we resolve on the new course requests XML file, we'll add the allocations from the updated solution to the updated input data XML file
	# to get the merged solution
