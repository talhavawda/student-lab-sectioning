...
requests separately and merge the obtained solution with the current solution to represent the updated solution.  
    ...
    along with the initial(current) solution file with the old course requests removed, and the second being to take the (updated) solution file 
    that was obtained by running the solver on the input data XML file generated by the first process (i.e. this solution file 
    contains the solutions of the new course requests) and merge it together with the  initial solution (i.e. the previous current 
    solution) that has the old course requests removed, resulting in an entire updated solution file.
        ...
        their new course requests), and I'd create its corresponding input data XML file inside SeparateModifiedInputProcessing.py
            ...
            the section allocation and add them to their respective course requests in (a copy of) the updated input data XML file - which 
            represents the current solution - to obtain the updated solution file. 
